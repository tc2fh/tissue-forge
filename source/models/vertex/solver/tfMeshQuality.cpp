...
static HRESULT MeshQuality_constructOperationsReconnection(
    ...
    // Scan + build the RNR (Okuda I<->H) candidate operations. The RNR-specific work -- the [I]/[H]
    // neighborhood walks, Condition-4 vetoes, slot de-dup, and op creation -- lives in
    // tfMeshReconnection.cpp (MeshReconnection_buildOperations). ops is indexed by surface id (both
    // triggers live on a Surface), matching this same id space, so MeshQuality_constructChains
    // serializes ops with overlapping touched surfaces. The generic chain-build + serial execution
    // below stay here with the rest of the MeshQuality scheduler.
    std::vector<MeshQualityOperation*> ops;
    MeshReconnection_buildOperations(mesh, passMask, reconnectLength, reconnectHysteresis, reconnectEnergyGate, ops);
...
std::string MeshQuality::str() const {
    ...
    ss << "stockVertexOps="     << (this->stockVertexOps ? "yes" : "no") << ", ";
    ss << "stockSurfaceOps="    << (this->stockSurfaceOps ? "yes" : "no") << ", ";
    ss << "stockBodyOps="       << (this->stockBodyOps ? "yes" : "no")  << ", ";
...
HRESULT MeshQuality::doQuality() { 
    ...
    // Stock (non-RNR) TissueForge quality checks: vertex merge/split, surface demote (+ 2D
    // collision), and body demote. Enabled by default to preserve historical behavior. Each pass is
    // gated by BOTH the master stockQualityOps flag and its own per-pass flag, so a vertex model can
    // run the native RNR reconnection alongside whichever stock passes it wants -- in particular,
    // keeping the vertex/surface passes while excluding body demotion (stockBodyOps), whose
    // degenerate body->vertex collapse is not robust on finite (free-surface) blocks.
    if(stockQualityOps) {

    // affectedBodiesImpl is filled by the surface pass and consumed by the body pass; it lives in
    // this shared scope so the per-pass gates below can be toggled independently.
    std::unordered_set<int> affectedBodiesImpl;

    // Vertex checks (stock vertex merge/split)
    ...
    // Surface checks (stock surface demote + 2D collision)
    ...
    // Body checks (stock body->vertex demotion; not finite-block-safe -- see stockBodyOps)
...
HRESULT MeshQuality::setStockVertexOps(const bool &_val) {
    stockVertexOps = _val;
    return S_OK;
}

HRESULT MeshQuality::setStockSurfaceOps(const bool &_val) {
    stockSurfaceOps = _val;
    return S_OK;
}

HRESULT MeshQuality::setStockBodyOps(const bool &_val) {
    stockBodyOps = _val;
    return S_OK;
}
...
namespace TissueForge::io {
    ...
    HRESULT toFile(const TissueForge::models::vertex::MeshQuality &dataElement, const MetaData &metaData, IOElement &fileElement) {
        ...
        TF_IOTOEASY(fileElement, metaData, "stockVertexOps", dataElement.getStockVertexOps());
        TF_IOTOEASY(fileElement, metaData, "stockSurfaceOps", dataElement.getStockSurfaceOps());
        TF_IOTOEASY(fileElement, metaData, "stockBodyOps", dataElement.getStockBodyOps());
    ...
    HRESULT fromFile(const IOElement &fileElement, const MetaData &metaData, TissueForge::models::vertex::MeshQuality *dataElement) {
        ...
        // before the native RNR port still deserialize.
        {
            ...
            bool stockVertexOps = true;
            auto _svItr = _rcChildren.find("stockVertexOps");
            if(_svItr != _rcChildren.end())
                ::TissueForge::io::fromFile(_svItr->second, metaData, &stockVertexOps);
            dataElement->setStockVertexOps(stockVertexOps);

            bool stockSurfaceOps = true;
            auto _ssItr = _rcChildren.find("stockSurfaceOps");
            if(_ssItr != _rcChildren.end())
                ::TissueForge::io::fromFile(_ssItr->second, metaData, &stockSurfaceOps);
            dataElement->setStockSurfaceOps(stockSurfaceOps);

            bool stockBodyOps = true;
            auto _sbItr = _rcChildren.find("stockBodyOps");
            if(_sbItr != _rcChildren.end())
                ::TissueForge::io::fromFile(_sbItr->second, metaData, &stockBodyOps);
            dataElement->setStockBodyOps(stockBodyOps);
