...
FVector3 SurfaceAreaConstraint::force(const Surface *source, const Vertex *target) {
    ...
    for(idx = 0; idx < svertices.size(); idx++) {
        ...
        if(!triNorm.isZero())
            sftotal += Magnum::Math::cross(
                triNorm.normalized(),
                meshRelativePosition(
                    svertices[idx == svertices.size() - 1 ? 0 : idx + 1]->getPosition(),
                    svertices[idx]->getPosition()
                )
            );
    }
    ...
    if(!triNorm.isZero())
        sftotal += Magnum::Math::cross(triNorm.normalized(), meshRelativePosition(scentroid, svertices[idxn]->getPosition()));
    triNorm = source->triangleNormal(idxp);
    if(!triNorm.isZero())
        sftotal -= Magnum::Math::cross(triNorm.normalized(), meshRelativePosition(scentroid, svertices[idxp]->getPosition()));
    ftotal += sftotal;
